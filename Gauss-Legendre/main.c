...
#include <stdlib.h>
#include <math.h>
#define FUNCAO (4/(1+pow(x,2)))
#define FUNCAO_EXIBIR "4/(1+x^2)"
...
double calcularResultado(double a, double b, int pontos, double erro){
    double result,resultAnterior;
    result = gaussLegendre(a,b,pontos);
    do{
        pontos++;
        resultAnterior = result;
        result = gaussLegendre(a,b,pontos);
    }while( fabs(result - resultAnterior ) > erro);
